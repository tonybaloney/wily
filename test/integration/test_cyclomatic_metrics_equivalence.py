...
def test_rust_cyclomatic_matches_radon() -> None:
    ...
    for name in rust_funcs:
        assert rust_funcs[name]["complexity"] == radon_funcs[name]["complexity"], \
            f"Complexity mismatch for {name}: Rust={rust_funcs[name]['complexity']}, Radon={radon_funcs[name]['complexity']}"
        assert rust_funcs[name]["lineno"] == radon_funcs[name]["lineno"], \
            f"Line number mismatch for {name}"
    ...
    for name in rust_classes:
        assert rust_classes[name]["complexity"] == radon_classes[name]["complexity"], \
            f"Class complexity mismatch for {name}"
        assert rust_classes[name]["real_complexity"] == radon_classes[name]["real_complexity"], \
            f"Class real_complexity mismatch for {name}"
