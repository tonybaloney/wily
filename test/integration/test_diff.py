...
def test_diff_only_shows_changed_functions(tmpdir):
    ...
    assert "src/test.py:unchanged_function" not in changed_entries, f"unchanged_function should not appear in diff, but found entries: {changed_entries}"

    # UnchangedClass and its method should NOT show in diff
    assert "src/test.py:UnchangedClass" not in changed_entries, f"UnchangedClass should not appear in diff, but found entries: {changed_entries}"
    assert "src/test.py:UnchangedClass.method" not in changed_entries, f"UnchangedClass.method should not appear in diff, but found entries: {changed_entries}"
...
def test_diff_no_duplicate_entries(tmpdir):
    ...
    for file_path, count in file_counts.items():
        assert count == 1, f"{file_path} appears {count} times in diff output, expected 1. Full output: {data}"
...
def test_diff_with_multiple_functions_only_changed_shown(tmpdir):
    ...
    assert "src/test.py:func_a" not in changed_files, f"func_a should not appear, got: {changed_files}"
    assert "src/test.py:func_c" not in changed_files, f"func_c should not appear, got: {changed_files}"
...
def test_diff_no_detail_raw_metrics_unchanged_when_mi_changes(tmpdir):
    ...
    if "loc" in metrics:
        ...
        assert loc_metric["before"] == loc_metric["after"], f"loc values should be identical when line count unchanged: {loc_metric}"

    if "sloc" in metrics:
        sloc_metric = metrics["sloc"]
        assert sloc_metric["before"] == sloc_metric["after"], f"sloc values should be identical when line count unchanged: {sloc_metric}"

    if "lloc" in metrics:
        lloc_metric = metrics["lloc"]
        assert lloc_metric["before"] == lloc_metric["after"], f"lloc values should be identical when line count unchanged: {lloc_metric}"
...
def test_diff_no_detail_shows_only_file_level_metrics(tmpdir):
    ...
    for entry in data:
        ...
        assert ":" not in file_path, f"--no-detail should not include function/class level entries, but found: {file_path}"
